...
public class validateNearestParking extends BaseTest
{
	String testCaseName="validateNearestParking";
	Xls_Reader xls;
	@Test(dataProvider="getData")
	public void validateNearestParkingTest(Hashtable<String,String> data) throws IOException
	{
        check=1;
		test = rep.startTest("validateNearestParkingTest");
		...
		List<String> listSlots = Arrays.asList(data.get("SlotNumber").split(","));
		System.out.println("slot size is "+listSlots.size());
		test.log(LogStatus.INFO, "Vacating the Parking for "+data.get("ParkingSlotName"));
		for (int i = 0; i < listSlots.size(); i++) {
			vacateParking(data.get("ParkingSlotName"),listSlots.get(i));
			if (getResponseCode().equalsIgnoreCase("200"))
			{
				test.log(LogStatus.INFO, "Vacated the parking with slot = "+listSlots.get(i));
			}
			else reportFailure("Failed");
		}
	    ...
	    if (Integer.parseInt(Body)==Integer.parseInt(listSlots.get(0)))
	    {
	    	reportPass("Passed");
	    }else reportFailure("Failed");
