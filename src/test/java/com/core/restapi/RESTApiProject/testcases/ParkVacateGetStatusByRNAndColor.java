...
public class ParkVacateGetStatusByRNAndColor extends BaseTest
{
	String testCaseName="ParkVacateGetStatusByRNAndColor";
	Xls_Reader xls;
	@Test(dataProvider="getData")
	public void ParkVacateGetStatusByRNAndColorTest(Hashtable<String,String> data) throws IOException
	{
        check=1;
		test = rep.startTest("ParkVacateGetStatusByRNAndColorTest");
	    ...
	    Body=getResponseBody();
			...
			if (getResponseCode().equalsIgnoreCase("200"))
			{
				test.log(LogStatus.INFO, "Got the slot number by registration number and color");
			}
			else reportFailure("Failed");
		...
		test.log(LogStatus.INFO, "Vacating the Parking for "+data.get("ParkingSlotName"));
		for (int i = 1; i <= Integer.parseInt(data.get("TotalSlotNumber")); i++) {
			vacateParking(data.get("ParkingSlotName"),String.valueOf(i));
			if (getResponseCode().equalsIgnoreCase("200"))
			{
				test.log(LogStatus.INFO, "Vacated the parking with slot = "+ i);
			}
			else reportFailure("Failed");
		}
		...
		getslotNumber(data.get("ParkingSlotName"),data.get("registrationNumber"),data.get("color"));
		  Body=getResponseBody();
			...
			{
				test.log(LogStatus.INFO, "Expected response code is 204 but actual is "+getResponseCode());
