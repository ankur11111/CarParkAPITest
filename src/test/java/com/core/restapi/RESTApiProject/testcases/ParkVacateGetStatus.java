...
public class ParkVacateGetStatus extends BaseTest
{
	String testCaseName="ParkVacateGetStatus";
	Xls_Reader xls;
	@Test(dataProvider="getData")
	public void ParkVacateGetStatusTest(Hashtable<String,String> data) throws IOException
	{
        check=1;
		test = rep.startTest("ParkVacateGetStatusTest");
		...
		{
			test.log(LogStatus.INFO, "Got the status");
		}
		else reportFailure("Failed");
		...
		parkingStatus(data.get("ParkingSlotName"));
		Body=getResponseBody();
		if (Body.equalsIgnoreCase("[]")&&getResponseCode().equalsIgnoreCase("200"))
		{
			reportPass("Passed");
		}
		else reportFailure("Failed");
